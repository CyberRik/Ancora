...
const STATE_STYLES: Record<NodeState, { dot: string; chip: string; label: string }> = {
  ...
  retrying: { dot: "bg-warning", chip: "bg-warning/15 text-warning", label: "retrying" },
...
function merge(cost: RunCost | null, retries: RetryAttempt[], live: RunLive | null): MergedNode[] {
  ...
  for (const act of live?.activities ?? []) {
    const nodeId = act.activity_type === "run_node" ? act.activity_id : act.activity_type;
...
function Rollup({ title, groups, total }: { title: string; groups: RunCost["by_node"]; total: number }) {
...
export function RunInspector({
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      api.getRunCost(runId, c.signal).then(setCost).catch(() => {});
      api.getRunRetries(runId, c.signal).then(setRetries).catch(() => {});
    };
    load();
    ...
    return (
      <section className="space-y-3">
        ...
        <div className="rounded-lg border bg-card p-4 text-sm text-muted-foreground">
          ...
          built-in node library (<code className="rounded bg-muted px-1">research_agent</code>)
          report cost and retries here as they run.
        </div>
      </section>
    );
  }
      ...
      <div className="grid gap-3 lg:grid-cols-[1fr_1fr]">
              ...
              >
                <span
                  className={cn(
                    ...
                    style.dot,
                    (n.state === "running" || n.state === "retrying") && "animate-pulse",
                  )}
                />
        ...
        <div className="space-y-3">
          {active ? (
            <div className="rounded-lg border bg-card p-4">
              ...
              <dl className="mt-3 grid grid-cols-2 gap-y-1 text-xs text-muted-foreground">
                <dt>Type</dt>
                <dd className="text-right font-mono text-foreground">{active.nodeType}</dd>
                <dt>Attempt</dt>
                <dd className="text-right tabular-nums text-foreground">{active.attempt}</dd>
                ...
                <dt>Cost</dt>
                <dd className="text-right tabular-nums text-foreground">{usd(active.usd)}</dd>
              </dl>
              ...
              {active.failures.length > 0 && (
                <div className="mt-4 space-y-2 border-t pt-3">
                  ...
                  {active.failures.map((f, i) => (
                    <div key={i} className="rounded-md border border-warning/30 bg-warning/5 p-2">
                      <div className="flex items-center justify-between text-[11px]">
                        <span className="font-mono">attempt {f.attempt}</span>
                        <span className={f.transient ? "text-warning" : "text-danger"}>
          ...
          {cost && cost.total_usd > 0 && (
            <>
              <Rollup title="Cost by model" groups={cost.by_model} total={cost.total_usd} />
              <Rollup title="Cost by provider" groups={cost.by_provider} total={cost.total_usd} />
            </>
          )}
        </div>
      </div>
